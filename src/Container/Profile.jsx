...
import { updateProfileRegister } from '../common/Apis/ApiService';
...
const Profile = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            let id = JSON.parse(localStorage.getItem('pr_id'))

            console.log({ username, profile_pic: file, user_id }, id, 12345)
            startLoading()
            const response = await updateProfileRegister(id, { username, profile_pic: file, user_id }, { 'Content-Type': 'multipart/form-data' })

            // const responsew = await axios.post(`${ENDPOINTS.updateProfileRegister}/${id}`, { username, profile_pic: file, user_id }, {
            //     headers: {
